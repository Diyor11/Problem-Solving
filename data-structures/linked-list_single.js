...
class SinglyLinkedList{
  ...
  pop() {
      ...
      let current = this.head // { val: 2, next: null }
      let newTail = current // { val: 1, next: Node { val: 2, next: null } }
      while (current.next) {
          newTail = current
          current = current.next
      }
      this.tail = newTail // { val: 1, next: null }
      this.tail.next = null // { val: 1, next: Node { val: 2, next: null } }
      this.length--
      if (this.length === 0) {
          this.head = null
          this.tail = null
  ...
  reverse(){
    ...
    this.head = this.tail // { val: 2, next: null }
    this.tail = node // { val: 1, next: { val: 2, next: null } }
    let next // null
    let prev = null // { val: 2, next: { val: 1, next: null } }
